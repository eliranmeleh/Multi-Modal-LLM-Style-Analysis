...
| N5 | **Modularity.** The LLM provider configurable via a single setting | `LLMProvider` protocol; no concrete provider imported in `pipeline/`; five backends behind one registry, the three live ones each behind an optional extra | `contract/test_provider_contract.py`, `contract/test_live_providers.py`; `integration` provider-swap test |
| N6 | **Auditability.** Every LLM call logged with prompt, response, model version and timestamp, so a run can be reproduced exactly | `llm/ledger.py` writes `calls.jsonl`; `llm/cache.py` content-addresses every call; each live provider records the parameters it actually put on the wire in `response.raw["wire"]`, including any the model refused to accept | `unit/test_ledger.py`, `unit/test_cache.py`, `contract/test_live_providers.py` |
| N7 | **No secret in a file, a log line or a commit.** Keys from the environment only | `llm/providers/_live.py` reads the key at construction and hands it straight to the SDK client without storing it; `.env` is git-ignored and loaded into the environment by the CLI | `contract/test_live_providers.py`; `unit/test_cli.py` dotenv tests; `conftest.py` strips the three key variables before every test |
