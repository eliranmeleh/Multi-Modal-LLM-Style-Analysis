...
| I20 | **A cache write that loses a race to an existing entry is treated as a success, not an error.** | Content addressing means two workers can legitimately compute the same key: the same passage occurring in two creations, or a retry overlapping its own first attempt. Whoever wins, the payload is equivalent, because the key is a function of the request. On Windows the loser gets a `PermissionError` from the rename; if the destination exists afterwards the entry is there and the write succeeded in every sense that matters. Failing instead would abort a wide run for no reason. | Yes |
| I21 | **`prompt_schema_version` lives on the request rather than in global configuration.** | `docs/PROMPTS.md` versions each template separately. Holding one global number would mean editing any template invalidated every cached response, turning a one-line prompt fix into a full re-run. On the request, a bump invalidates exactly the entries that used that template. | Yes |
| I22 | **The cache key excludes the request's `tag`.** | The tag records what a call is *for* — profile, rewrite, control — not what was asked. Including it would mean the same prompt issued from two places paid for the same answer twice, and would break the control experiment's pairing, where Mode A and Mode B must be distinguishable by prompt rather than by label. | Yes |
| I23 | **The preamble pattern in `docs/SPEC.md` Step 3 was corrected to match the contraction "here's".** | As transcribed it read `here (is|'s)`, with a space before the group, so it matched "here 's" and not "here's" — the commoner opening of the two. This is a transcription defect in an `[IMPL]` detail with unambiguous intent, not a method question, so it was corrected in place rather than raised. It matters because an unstripped preamble does not fail loudly: it is measured as part of the passage and inflates that chunk's delta, which is precisely the false positive the validation exists to prevent. | Yes |
